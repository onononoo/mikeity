...
def write_all(s, out):
    ...
    everything = dict(
        ...
        glossary=[dict(slug=t.slug, term=t.term, definition=t.definition, see=t.get("see", []),
                       used_in=[e.slug for e in t.used_in]) for t in s.terms],
        regions=[dict(slug=r.slug, name=r.name, description=r.get("description", "")) for r in s.regions],
        ...
        religions=[dict(slug=r.slug, name=r.name, began=r.began, followers=r.followers,
                        founder=r.founder, scriptures=r.scriptures, places=r.places,
                        summary=r.summary, creation=r.creation, age=r.age,
                        story=strip_tags(r.doc.html),
                        events=[dict(id=ev.id, kind=ev.kind, when=ev.shown, text=ev.text,
                                     start=ev.when.start if ev.when else None,
                                     end=ev.when.end if ev.when else None) for ev in r.events],
                        people=[dict(slug=p.slug, name=p.name, dates=p.dates, kind=p.kind, role=p.role,
                                     about=p.about) for p in r.people],
                        glossary=[dict(slug=t.slug, term=t.term, definition=t.definition) for t in r.terms])
                   for r in s.religions],
    )
